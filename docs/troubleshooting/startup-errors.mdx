...
## 0. Make sure you're on a version that allows mods.
Some specific versions of VotV have errors or crash when using mods. For example, earlier releases of 0.9.0 have all come shipped with an anti-cheat system built-in that immediately crashes the game when it detects mods or the modloader is installed, so these versions cannot be played with mods.\
...
It's good to double-check, so make sure you've followed all the directions in the installation guides ([Mod Manager](../installing-mods/install-modman)/[Manual Install](../installing-mods/install-manual)) and have not missed any steps or important notes.
