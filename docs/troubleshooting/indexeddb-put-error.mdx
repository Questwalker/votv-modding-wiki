...
This is an bug with modmanagers that can occasionally happen, typically when attempting to change the VotV folder in the settings. To fix the error, close the mod manager you are using, then..

* If you use r2modman, go to the folder `%appdata%\r2modman` and delete the IndexedDB folder.
* If you use thunderstore, go to the folder `%localappdata%\Overwolf\CefBrowserCache\Default` and delete the IndexedDB folder.
